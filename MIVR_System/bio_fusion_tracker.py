...
# 引入自適應卡爾曼
try:
    from adaptive_kalman import AdaptiveKalmanPoint, AKFConfig
except ImportError:
    raise ImportError("需要 adaptive_kalman.py，請確認在同一目錄")
...
class BioFusionTracker:
    ...
    def _estimate_ankle(self, side: str, ank_id: int,
        ...
        if ank_conf >= CONF_VISIBLE:
            sx, sy = self._akf[side].update(raw_x, raw_y)
        ...
        if ank_conf >= CONF_LOW:
            ...
            # 加權融合
            w_direct = ank_conf
            w_arm    = arm_weight * (1.0 - ank_conf) * 0.5
            ...
            sx, sy = self._akf[side].update(fused_x, fused_y)
            self._missing[side] = 0
            self._last_valid[side] = (sx, sy)
            return AnkleEstimate(
                x=sx, y=sy,
        ...
        if hip_conf > CONF_VISIBLE:
            ...
            if extrap:
                ex, ey = extrap
                sx, sy = self._akf[side].update(ex, ey)
                self._last_valid[side] = (sx, sy)
                return AnkleEstimate(
                    x=sx, y=sy,
                    confidence=0.35,
...
class MultiBioFusionTracker:
    ...
    def process_person(self, person_id: int,
        ...
        return self._trackers[person_id].process(kpts, img_h, img_w)
