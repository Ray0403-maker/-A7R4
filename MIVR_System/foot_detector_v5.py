"""
MIVR-CEIQ Foot Detector v5  [漏洞修正版]
foot_detector_v5.py
...
try:
    ...
    from bio_fusion_tracker_v2 import (
    ...
    BIO_AVAILABLE = False
    print(f"[WARN] bio_fusion_tracker_v2 not found ({e})")
...
def detect(camera_index, save_csv_flag):
    ...
    if BIO_AVAILABLE:
        akf_cfg = AKFConfig(
            q_base=0.3, q_max=1200.0,
            # ── 量測噪音：資料驅動，零相機假設 ──
            # 系統從每個點的實際殘差自己學噪音水平
            # 不規定相機角度，俯角/平視/任何架設方式都自動適應
            r_noise_base=5.0, r_noise_max=60.0,
            r_residual_window=15, r_residual_gain=0.6,
            r_dist_scale=0.0,   # 0=不用畫面位置假設
            innovation_window=6, innovation_gain=4.5, speed_gain_exp=1.5,
            velocity_decay=0.80, still_threshold=1.8, still_damping=0.03,
            outlier_gate_min=40.0, outlier_gate_vel=3.5, outlier_max_angle=110.0,
            median_window=3, ewma_alpha_move=0.85, ewma_alpha_still=0.20,
            cf_cutoff_move=2.5, cf_cutoff_still=1.0, max_missing=15
        )
    ...
    while True:
        ...
        # [Bug-3 修正] 用 track() 取得穩定 ID（內建 ByteTrack）
        if track_on:
            res = model.track(frame, conf=conf_th, iou=IOU_THRESHOLD,
        ...
        if key in (ord('q'),ord('Q')): break
            ...
            ts=int(time.time()); cv2.imwrite(f"{DATA_DIR}/shot_{ts}.jpg",frame)
            print(f"  [shot] {ts}")
...
def main():
    p=argparse.ArgumentParser(description="MIVR-CEIQ Foot Detector v5")
    p.add_argument("--camera",type=int,default=0,help="-1=auto scan")
    p.add_argument("--no-csv",action="store_true")
    a=p.parse_args()
    print("="*58)
    print("  MIVR-CEIQ Foot Detector v5  [bug-fixed]")
    print("="*58)
